...
def send_telegram(photo_path="alert.png"):
    try:
        my_token = "6142812078:AAEq50XHRtLXdpCyONNeo1uP-rIEKPZc1CY"
        bot = telegram.Bot(token=my_token)
        bot.sendPhoto(chat_id="6253730735", photo=open(photo_path, "rb"), caption="Có xâm nhập, nguy hiêm!")
...
class YoloDetect():
    def __init__(self, detect_class="person", frame_width=1280, frame_height=720):
        # Parameters
        self.classnames_file = "model/classnames.txt"  # dg dan den noi chua cac dt
        self.weights_file = "model/yolov4-tiny.weights" #chua trong so cua mohinh yolo
        self.config_file = "model/yolov4-tiny.cfg" #cau hinh cua yolo
    ...
    def alert(self, img):
        ...
        if (self.last_alert is None) or (
            ...
            cv2.imwrite("alert.png", cv2.resize(img, dsize=None, fx=1, fy=1))
